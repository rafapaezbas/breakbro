window.onload = () => {
    ...
    const listFiles = () => {
        ...
        fetch('http://www.pinkumandrill.com:38081/file', {method: "GET", headers: headers}).then(res => res.json()).then(files => files.map(appendFileToList));
    };
    ...
    document.getElementById("file-upload").onchange = () => {
        ...
        fetch('http://www.pinkumandrill.com:38081/file', {method: "POST", headers: headers, body: formData}).then(successUpload(file.name),errorUpload);
    };

    document.getElementById("start-stream-button").onclick = () => {
        const headers = getHeaders();
        fetch('http://www.pinkumandrill.com:38081/streamer/init', {method: "GET", headers: headers}).then(console.log);
    };
